...
def run_vandermonde(space) -> None:
    ...
    time_jax = timeit.timeit(
        "space.vandermonde(x)", number=M, globals={**globals(), **locals()}
    )
    ...
    time_np = timeit.timeit(
        f"npfun(xn, {N})", number=M, globals={**globals(), **locals()}
    )
...
def run_evaluate(space) -> None:
    ...
    space.evaluate(x, c)
    time_jax = timeit.timeit(
        "space.evaluate(x, c)", number=M, globals={**globals(), **locals()}
    )
    print(f"{'Jax':20s} {time_jax:.4e}")
    ...
    time_np = timeit.timeit(
        "npfun(xn, cn)", number=M, globals={**globals(), **locals()}
    )
    print(f"{'Numpy':20s} {time_np:.4e}")
