...
from jaxfun.la import DiaMatrix, diakron
...
for n in SIZES:
    ...
    X = jax.random.normal(key, (n * n, BATCH))

    # ── solve vector ──────────────────────────────────────────────────────────
    t_dia = bench(lambda: lu_dia.solve(x), REPEATS)
    t_mat = bench(lambda: lu_mat.solve(x), REPEATS)
    print(f"{'solve (pre-LU) vec':<20} {n:>6}  {fmt(t_dia)}  {fmt(t_mat)}  {speedup(t_dia, t_mat)}")

    # ── solve matrix (k=32 cols) ──────────────────────────────────────────────
    t_dia = bench(lambda: lu_dia.solve(X), REPEATS)
    t_mat = bench(lambda: lu_mat.solve(X), REPEATS)
    print(f"{'solve (pre-LU) mat':<20} {n:>6}  {fmt(t_dia)}  {fmt(t_mat)}  {speedup(t_dia, t_mat)}")

    # ── full solve (factorisation included) ───────────────────────────────────
    t_dia = bench(lambda: A.solve(x), REPEATS)
    t_mat = bench(lambda: M.solve(x), REPEATS)
    print(f"{'solve (full) vec':<20} {n:>6}  {fmt(t_dia)}  {fmt(t_mat)}  {speedup(t_dia, t_mat)}")
