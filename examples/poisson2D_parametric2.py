...
C = get_CoordSys(
    ...
    sp.Lambda(
        ...
        (
            ...
            + sp.sqrt((eta-1)*(eta+1)/4) * (1 + xi) / 2
            + sp.sqrt((eta-1)*(eta+1)/4) * (1 - xi) / 2,
            sum([phi[i] * nodes[i][1] for i in range(4)])
            + sp.sqrt((xi-1)*(xi+1)/4) * (1 + eta) / 2
            + sp.sqrt((xi-1)*(xi+1)/4) * (1 - eta) / 2,
        ),
    ),
    ...
    & sp.Q.positive(eta - 1)
    & sp.Q.positive(xi - 1)
    & sp.Q.positive(eta**2 - 1)
    & sp.Q.positive(xi**2 - 1)
)
...
yc = jnp.broadcast_to(yc, xc.shape)
