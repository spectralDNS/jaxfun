...
def test_lsqr_loss_i(flax_func):
    ...
    loss_i = lsqr.loss_i(flax_func.module, 0)
    xs = lsqr.residuals[0].x
    assert loss_i.shape == ()  # scalar loss
    assert jnp.allclose((flax_func(xs) - 1.0) ** 2, loss_i)
...
def test_lsqr_compute_global_weights(flax_func):
    ...
    weights = lsqr.compute_global_weights(flax_func.module)
...
def test_lsqr_update_global_weights(flax_func):
    ...
    _ = lsqr.loss_with_gw(flax_func.module, gw)  # Initialize Js
    old_weights = jnp.ones(len(lsqr.residuals), dtype=float)
    new_weights = lsqr.update_global_weights(flax_func.module, old_weights, 0.5)
...
def test_residual_call_no_flaxfunctions():
    ...
    f = sp.Integer(5)
    residual = Residual(f, x)
    result = residual(x, jnp.array(0), None)
...
def test_residual_eval_compute_grad(flax_func):
    ...
    result = residual.eval_compute_grad(flax_func.module, id(x))
...
def test_residual_loss_compute_grad(flax_func):
    x = jnp.array([[1.0, 2.0]])
    residual = Residual(flax_func, x)
    result = residual.loss_compute_grad(flax_func.module, id(x))
...
def test_loss_value_and_grad_and_JTJ(flax_func):
    ...
    gw = jnp.array([1.0])
    loss, grad, JTJ = lsqr.value_and_grad_and_JTJ(flax_func.module, gw)
    assert isinstance(loss, jnp.ndarray)
    assert loss.shape == ()
...
def test_loss_JTJ(flax_func):
    ...
    lsqr = Loss((flax_func, x))
    gw = jnp.array([1.0])
    JTJ = lsqr.JTJ(flax_func.module, gw)
    assert isinstance(JTJ, jnp.ndarray)
    assert JTJ.ndim == 2
