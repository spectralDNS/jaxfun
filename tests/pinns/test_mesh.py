...
def test_unitsquare_uniform_inside_matches_meshgrid():
    Nx, Ny = 4, 5
    us = UnitSquare(Nx=Nx, Ny=Ny)

    pts = us.get_points_inside_domain("uniform")
...
def test_unitsquare_legendre_inside_is_tensor_product():
    ...
    pts = us.get_points_inside_domain("legendre")
    x = (1 + leggauss(Nx)[0]) / 2
    y = (1 + leggauss(Ny)[0]) / 2
    expected = _meshgrid_flatten(x, y)
...
def test_unitsquare_chebyshev_inside_is_tensor_product():
    ...
    pts = us.get_points_inside_domain("chebyshev")
    x = _cheb_nodes(Nx)
    y = _cheb_nodes(Ny)
    expected = _meshgrid_flatten(x, y)
...
def test_unitsquare_boundary_legendre_and_chebyshev_basic():
    ...
    for kind in ("legendre", "chebyshev", "random", "uniform"):
        bd_c = us.get_points_on_domain(kind, corners=True)
        bd_nc = us.get_points_on_domain(kind, corners=False)
...
def test_unitsquare_random_reproducible_with_rng_key():
    Nx, Ny = 8, 3
    rngs = nnx.Rngs(123)
    us1 = UnitSquare(Nx=Nx, Ny=Ny, key=rngs())
    us2 = UnitSquare(Nx=Nx, Ny=Ny, key=nnx.Rngs(123)())

    p1 = us1.get_points_inside_domain("random")
    p2 = us2.get_points_inside_domain("random")
...
def test_unitline_points_and_weights():
    ...
    # inside points
    pu = ul.get_points_inside_domain("uniform")
    pl = ul.get_points_inside_domain("legendre")
    pc = ul.get_points_inside_domain("chebyshev")
    pr = ul.get_points_inside_domain("random")
    ...
    # weights
    w_u = ul.get_weights_inside_domain("uniform")
    w_r = ul.get_weights_inside_domain("random")
    w_l = ul.get_weights_inside_domain("legendre")
    w_c = ul.get_weights_inside_domain("chebyshev")
    ...
    assert ul.get_weights_on_domain("uniform") == 1


def test_line_affine_mapping():
    N = 5
    left, right = -2.5, 3.0
    line = Line(N=N, left=left, right=right)

    p = line.get_points_inside_domain("uniform")
    assert p.shape == (N, 1)
    # map should be within interval
    assert p.min() > left
    assert p.max() < right

    bd = line.get_points_on_domain("uniform").squeeze()
...
def test_unitsquare_inside_points_all_kinds():
    ...
    for kind in ("uniform", "legendre", "chebyshev", "random"):
        pts = us.get_points_inside_domain(kind)
        assert pts.shape == (Nx * Ny, 2)
...
def test_unitsquare_boundary_points_and_corners():
    ...
    # with corners
    bd = us.get_points_on_domain("uniform", corners=True)
    ...
    # without corners
    bd_nc = us.get_points_on_domain("uniform", corners=False)
    assert bd_nc.shape == (2 * Nx + 2 * Ny, 2)
...
def test_unitsquare_weights_inside_and_boundary():
    ...
    # inside weights
    wi_u = us.get_weights_inside_domain("uniform")
    wi_r = us.get_weights_inside_domain("random")
    wi_l = us.get_weights_inside_domain("legendre")
    wi_c = us.get_weights_inside_domain("chebyshev")
    ...
    for kind in ("uniform", "legendre", "chebyshev", "random"):
        for corners in (False, True):
            wb_l = us.get_weights_on_domain(kind, corners=corners)
            base_len = 2 * Nx + 2 * Ny
...
def test_rectangle_maps_unitsquare():
    Nx, Ny = 2, 3
    rect = Rectangle(Nx=Nx, Ny=Ny, left=-2.0, right=4.0, bottom=1.5, top=3.0)

    pts_in = rect.get_points_inside_domain("uniform")
    assert pts_in.shape == (Nx * Ny, 2)
    assert jnp.all((pts_in[:, 0] > -2.0) & (pts_in[:, 0] < 4.0))
    assert jnp.all((pts_in[:, 1] > 1.5) & (pts_in[:, 1] < 3.0))

    bd = rect.get_points_on_domain("uniform", corners=True)
    assert bd.shape == (2 * Nx + 2 * Ny + 4, 2)
...
def test_annulus_polar_inside_and_boundary_uniform():
    ...
    Nx = Ny = 4
    ri, ro = 2.0, 5.0
    ap = AnnulusPolar(Nx=Nx, Ny=Ny, radius_inner=ri, radius_outer=ro)

    for kind in ("uniform", "random"):
        pts_in = ap.get_points_inside_domain(kind)
        assert pts_in.shape == (Nx * Ny, 2)
        r = pts_in[:, 0]
        theta = pts_in[:, 1]
        ...
        bd = ap.get_points_on_domain(kind, corners=False)
...
def test_annulus_cartesian_inside_and_boundary_uniform():
    Nx = Ny = 3
    ri, ro = 1.0, 2.0
    ann = Annulus(Nx=Nx, Ny=Ny, radius_inner=ri, radius_outer=ro)

    pts_in = ann.get_points_inside_domain("uniform")
    assert pts_in.shape == (Nx * Ny, 2)
    # radii in [ri, ro]
    radii = jnp.linalg.norm(pts_in, axis=1)
    assert jnp.all((radii > ri) & (radii < ro))

    bd = ann.get_points_on_domain("uniform", corners=False)
    assert bd.shape == (2 * Ny, 2)
